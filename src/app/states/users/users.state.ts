...
export class UsersState {
  ...
  constructor(
    ...
    private schema: UserFireStoreService
  ) {
  }
  ...
  onUserCreate(ctx: StateContext<IUsersStateModel>, action: UserCreateAction) {
    ...
    return from(this.schema.create(form, form.id)).pipe(
  ...
  onGetUsers(ctx: StateContext<IUsersStateModel>) {
    ...
    if (!this.GetUserSubscription) {
      ...
      this.GetUserSubscription = this.schema.collection$(ref => ref.orderBy('createDate', 'desc')).pipe(
        tap(items => {
  ...
  onGetUserPage(ctx: StateContext<IUsersStateModel>) {
    ...
    if (!this.GetUserSubscription) {
      ctx.dispatch(new UserSetAsLoadingAction());
      this.GetUserSubscription = this.schema.collection$(ref => ref.limit(pageSize).orderBy(orderByField, 'desc')).pipe(
        tap(model => {
          if (!model.length) {
            return false;
          }
